...
import { DatumFindManyArgs } from "../../datum/base/DatumFindManyArgs";
import { Datum } from "../../datum/base/Datum";
...
export class CountryControllerBase {
  ...
  async findManyData(
    ...
    const query = plainToClass(DatumFindManyArgs, request.query);
    const permission = this.rolesBuilder.permission({
      role: userRoles,
      action: "read",
      possession: "any",
    ...
    const results = await this.service.findData(params.id, {
      ...query,
      select: {
        address: true,
  ...
  async updateData(
    @common.Param() params: CountryWhereUniqueInput,
    @common.Body() body: DatumWhereUniqueInput[],
    @nestAccessControl.UserRoles() userRoles: string[]
  ): Promise<void> {
    const data = {
      data: {
